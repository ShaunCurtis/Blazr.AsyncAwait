...
It has a static nullable `Current` property, which returns an SC for the current thread.  That tells us that a `Thread` object may have an SC attached to it, but as the return value is nullable, it may be null.
...
It has a static Method `SetSynchronizationContext` which sets the SC for the current value to the provided instance.  The argument is nullable, so se can clear the thread's SC. 
